...
export default function PoliticaPrivacidad() {
    ...
    const [textosPolitica, settextosPolitica] = useState([]);

    const API = process.env.NEXT_PUBLIC_API_URL

    async function llamarTituloPolitica(){
        try {
...
useEffect(() => {
    llamarTituloPolitica()
},[])




    async function llamarTextoPolitica(){
        try {
            ...
            const data = await res.json();
            let textoPoliticasDevoluciones = [];
            if(Array.isArray(data)){
                textoPoliticasDevoluciones = data.find(textoBuscado => textoBuscado.id_Textos === 5);
                return settextosPolitica(textoPoliticasDevoluciones);
            }else{
